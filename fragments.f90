...
MODULE Fragments
  ...
  INTEGER  :: flambda(mnof)           !< Lambda number of the fragments.
  INTEGER  :: fnstmax(mnof)            !< Last index number of the wave functions of the fragments.
  INTEGER  :: fnpmin(3,mnof)           !< First index numbers for protons and neutrons of the fragments.
  INTEGER  :: fnpsi(3,mnof)            !< Last index numbers for protons and neutrons of the fragments.
  INTEGER  :: fnumber(3,mnof)          !< gives the number of wave functions in the
  ...
  !!into account.
  INTEGER  :: fnewnpmin(3,mnof)        !<are the starting indices for each fragment's wave 
  !!functions in the total set of wave functions combining all the fragments.
  INTEGER  :: fnewnpsi(3,mnof)         !<are the ending indices for each fragment's wave 
  !!functions in the total set of wave functions combining all the fragments.
  ...
  SUBROUTINE getin_fragments
    ...
    DO i=1,nof
       OPEN(UNIT=scratch,FILE=filename(i),STATUS='old',FORM=&
            'unformatted')
       READ(scratch) iter,time,forcename,fnstmax(i),fnneut(i),fnprot(i),flambda(i),&
   ...
   npsi(3)=SUM(fnumber(3,:))
   nstmax=npsi(3)+npsi(2)+npsi(1)
    npmin(1)=1
    npmin(2)=npsi(1)+1
    npmin(3)=npsi(2)+1+npsi(1)
    npsi(3)=nstmax
    npsi(2)=npsi(1)+npsi(2)
    fnewnpmin(2,:)=fnewnpmin(2,:)+npmin(2)-1
    fnewnpsi(2,:)=fnewnpsi(2,:)+npmin(2)-1
    fnewnpmin(3,:)=fnewnpmin(3,:)+npmin(3)-1
    fnewnpsi(3,:)=fnewnpsi(3,:)+npmin(3)-1
    charge_number=nprot
  ...
  SUBROUTINE read_one_fragment(iff)
    ...
    DO iq=1,3
       ...
       IF(iq==2.AND..NOT.multifile) THEN
          ...
          END DO
       END IF
       ! skip unoccupied proton states in one-file case
       IF(iq==3.AND..NOT.multifile) THEN
          DO nst=fnumber(2,iff)+1,fnpsi(2,iff)-fnpsi(1,iff)
             READ(scratch)
          END DO
       END IF
  ...
  SUBROUTINE boost_fragment(iff)  
    ...
    DO iq=1,3  
       ...
       ELSE
          h2mq=h2ml
       ENDIF
       akf=sb*SQRT(1.0D0/h2mq*ABS(tb)/fmass(iff))  
