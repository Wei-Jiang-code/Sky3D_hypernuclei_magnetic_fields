...
MODULE Moment
  ...
  IMPLICIT NONE
  PRIVATE
  REAL(db) :: pnr(3)    !<the numbers of neutrons <tt>pnr(1)</tt>=\f$ N \f$ and 
  ...
  REAL(db) :: cm(3,3)   !<the center of mass vectors of
  ...
  REAL(db) :: pcm(3,3)  !<the integrated momentum vectors, not containing
  !!the nucleon mass. They thus correspond to an integral over the
  !!current density only and have a dimension of velocity: \f$ c \f$.
  REAL(db) :: rms(3)    !< these are the root
  !!mean-square radii of neutron, proton, and total mass distribution.  Dimension: fm.
  REAL(db) :: rmstot    !< these are the root mean-square radii of the total
  !!mass distribution.  Dimension: fm.
  REAL(db) :: q20(3)    !< the \f$ m=0 \f$ components of the spherical quadrupole tensor 
  ...
  REAL(db) :: q22(3)    !< the \f$ m=2 \f$ components of the spherical quadrupole tensor 
  ...
  !!\f$ Q_{20} \f$ in the principal-axes frame for the total mass distribution.
  REAL(db) :: x2m(3,3)  !<the vectors of second moments of the radii in the three coordinate
  ...
  PUBLIC :: pnr,pnrtot,cm,cmtot,pcm,rmstot,beta,gamma, &
       moments,moment_print,moment_shortprint,rms
CONTAINS
!---------------------------------------------------------------------------  
...
!--------------------------------------------------------------------------- 
  SUBROUTINE moments
    ...
    DO iq=1,3  
       ...
       ENDDO
    ENDDO
    pnrtot=pnr(1)+pnr(2)+pnr(3)
    cmtot=(cm(:,1)+cm(:,2)+cm(:,3))/pnrtot  
    DO iq=1,3
       cm(:,iq)=cm(:,iq)/pnr(iq)
    ENDDO
    !***********************************
    ...
    ENDDO
    rmstot=SQRT((rms(1)+rms(2)+rms(3))/pnrtot)
    rms=SQRT(rms/pnr)  
    x2mtot=(x2m(:,1)+x2m(:,2)+x2m(:,3))/pnrtot  
    DO iq=1,3
       x2m(:,iq)=x2m(:,iq)/pnr(iq)
    ENDDO
    qmtot=qmat(:,:,1)+qmat(:,:,2)+qmat(:,:,3)
    ...
    CALL q2diag(qmat(:,:,3),q20(3),q22(3),'Lambda   ')
...
!--------------------------------------------------------------------------- 
  SUBROUTINE moment_print
    INTEGER :: iq
    CHARACTER(11),PARAMETER :: Name(3)=(/ '  Neutron: ','   Proton: ','   Lambda: '/)
