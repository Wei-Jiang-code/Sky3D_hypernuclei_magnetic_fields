...
!--------------------------------------------------------------------------- 
  SUBROUTINE write_wavefunctions
    ...
    IF(mpi_myproc==0) THEN
       ...
       WRITE(scratch2) iter,time,f%name,nstmax,nneut,nprot,nlambda,number,npsi, &
  ...
  SUBROUTINE sp_properties
    ...
    DO nst=1,nstmax
       ...
       IF(TFFT) THEN
          ...
          CALL cdervy(pst,psy)  
          CALL cdervz(pst,psz)  
          CALL laplace(pst,psw,iq=isospin(nst))  
       ELSE
       ...
       IF(isospin(nst)<3) THEN
          sp_kinetic(nst)=wxyz*f%h2m(isospin(nst))*kin
       ELSE
          sp_kinetic(nst)=wxyz*h2ml*kin
       ENDIF
