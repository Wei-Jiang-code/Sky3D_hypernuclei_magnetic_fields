...
MODULE Static
  ...
  IMPLICIT NONE
  LOGICAL  :: tdiag=.True.    !< if \c true, there is a diagonalization of
  ...
  LOGICAL  :: tvaryx_0=.True. !< it <tt>.TRUE.</tt> the parameter \f$ x_0 \f$
...
!--------------------------------------------------------------------------- 
  SUBROUTINE getin_static
    NAMELIST/static/ tdiag,tlarge,maxiter, &
         radinx,radiny,radinz,serr,x0dmp,e0dmp,nneut,nprot,nlambda,npsi,tvaryx_0,b_d17
    npsi=0
    READ(5,static)
    IF(nof<=0) THEN
       IF(npsi(1)==0) THEN  
      ...
      IF(npsi(3)==0) THEN  
       ...
       IF(nneut>npsi(1).OR.nprot>npsi(2).OR.nlambda>npsi(3)) & 
            STOP 'Particle & state numbers in conflict'
       nstmax=npsi(1)+npsi(2)+npsi(3)
       charge_number=nprot  
       mass_number=nneut+nprot+nlambda  
       x0dmpmin=x0dmp
  ...
  if(nof>0) then
     ...
     IF(MOD(npsi(2),2)/=0) npsi(2)=npsi(2)+1
       npsi(3)=nlambda
         IF(MOD(npsi(3),2)/=0) npsi(3)=npsi(3)+1  
      nstmax=npsi(1)+npsi(2)+npsi(3)    
      charge_number=nprot  
      mass_number=nneut+nprot+nlambda  
      x0dmpmin=x0dmp
...
!--------------------------------------------------------------------------- 
  SUBROUTINE init_static
    IF(wflag) THEN
       ...
       WRITE(*,"(3(A,I4))") ' Lambdas :',nlambda,' in ',npsi(3)-npsi(2),' levels'
       ...
       CALL start_protocol(converfile, &
            '# Iter   Energy   nrms  prms  rms   &
            &beta  gamma     T        E0        E1     E2       E3     &
            &Els     E0odd   E1odd   E3odd   Elsodd   Efin   Eeff     Ec    Epair    Espin   &
            &Eorb    eA**2   espin_lambda   ehf_lambda')
    ENDIF
...
!--------------------------------------------------------------------------- 
  SUBROUTINE statichf
    ...
    !count意味着多少个需要交换的能级
    INTEGER :: iq,nst,firstiter,ix,iy,iz,bin,n_r=100
    REAL(db) :: denerg,temp,rho_min=1.0d-6

    real(db) :: dr, r_max,rho_r(100), r_count(100),r_bin,r_center(100)
   REAL(db),PARAMETER :: addnew=0.2D0,addco=1.0D0-addnew
   REAL(db), ALLOCATABLE :: wq(:,:,:,:),tdens_old(:,:,:,:,:),sdens_old(:,:,:,:,:),current_old(:,:,:,:,:)
   REAL(db), ALLOCATABLE :: rho_old(:,:,:,:),tau_old(:,:,:,:)
   CHARACTER(len=200) :: fname,fnameL,fnameLr,fnameN
    ! Step 1: initialization
   IF(tdiag) ALLOCATE(hmatr(nstmax,nstmax))
   ALLOCATE(wq(nx,ny,nz,2),tdens_old(nx,ny,nz,3,2),sdens_old(nx,ny,nz,3,2),current_old(nx,ny,nz,3,2), &
          rho_old(nx,ny,nz,3),tau_old(nx,ny,nz,3))
   wq=0.0D0
   tdens_old=0.0D0
   sdens_old=0.0D0
   current_old=0.0D0
...
!wocc(npmin(3)+2) = 1.0D0
    ...
    DO nst=1,nstmax
       CALL add_density(isospin(nst),wocc(nst),psi(:,:,:,:,nst), &
            rho,tau,current,sdens,sodens,tdens)  
    ENDDO
    CALL skyrme
    ...
    Iteration: DO iter=firstiter,maxiter  
      ...
      !$OMP END PARALLEL DO
       ...
       !$OMP REDUCTION(+:rho, tau, current, sdens, sodens,tdens)
       DO nst=1,nstmax
          CALL add_density(isospin(nst),wocc(nst),psi(:,:,:,:,nst), &
               rho,tau,current,sdens,sodens,tdens)  
       ENDDO
       !$OMP END PARALLEL DO
       IF(taddnew) THEN
          rho=addnew*rho+addco*rho_old
          tau=addnew*tau+addco*tau_old
          tdens = addnew/2.0 * tdens + (1.0-addnew/2.0) * tdens_old
          current = addnew/2.0 * current + (1.0-addnew/2.0) * current_old
         sdens = addnew * sdens + addco * sdens_old
       ENDIF
       CALL skyrme
       ! calculate and print information
       IF((mprint>0.AND.MOD(iter,mprint)==0).or.(iter==maxiter).or.(sumflu/nstmax<serr.AND.(iter>1))) THEN
          CALL sp_properties
          CALL sinfo
...
do iq=1,2
  ...
  DO iy = 1, ny
    DO ix = 1, nx
      IF ( rho(ix,iy,iz,1) > rho_min ) THEN
        wq(ix,iy,iz,iq) = sdens(ix,iy,iz,3,iq) / rho(ix,iy,iz,iq)
      ELSE
        wq(ix,iy,iz,iq) = 0.0_db
      END IF
    END DO
  END DO
END DO       
...
do iz = 1, nz
  do iy = 1, ny
    do ix = 1, nx
      ...
      if (bin > n_r) bin = n_r
      ! 累加密度
      rho_r(bin) = rho_r(bin) + rho(ix, iy, iz, 3)
...
! 求每个 bin 平均密度
do bin = 2, n_r
  if (r_count(bin) > 0.0) rho_r(bin) = rho_r(bin) / r_count(bin)
...
do bin = 1, n_r
   WRITE(99,'(F12.6,1X,F12.6)') r_center(bin), rho_r(bin)
end do
CLOSE(99)
!=========================================================
    ...
    END DO Iteration
      ...
      IF(ALLOCATED(tdens_old)) DEALLOCATE(tdens_old)
      IF(ALLOCATED(sdens_old)) DEALLOCATE(sdens_old)
      IF(ALLOCATED(current_old)) DEALLOCATE(current_old)
...
esf=spe 
    ...
    IF(e0dmp>0.0D0) THEN  
       ...
       x0act=x0dmp
       IF(TFFT) THEN
          CALL laplace(ps1,ps2,e0inv=e0dmp,iq=iq)  
       ELSE
...
!--------------------------------------------------------------------------- 
  SUBROUTINE sinfo
    ...
    WRITE(scratch,'(1x,i5,f11.3,3f6.3,2f7.3,2f10.2,17f8.2)') &
         iter,ehfint,rms(1),rms(2),rmstot,beta,gamma,&
         ehft,ehf0,ehf1_even,ehf2,ehf3,ehfls,e0odd,ehf1_odd,e3odd,ehflsodd,&
         efinodd,Eeffodd,ehfc,epair(1)+epair(2),Zeeman_spin,Zeeman_orb,Zeeman_A2,&
         Zeeman_lambda,ehf_lambda
    CLOSE(scratch)
    ...
    DO il=1,nstmax
      ...
      elseif(il==npmin(3)) THEN
          WRITE(*,'(A)') ' Lambda Single Particle States:',header
       END IF
...
!--------------------------------------------------------------------------- 
  SUBROUTINE harmosc
    ...
    wocc(npmin(3):npmin(3)+nlambda-1)=1.D0
    !*************************************************************************
