...
MODULE Densities
  ...
  !the time-odd spin-kinetic density, defined as
  REAL(db), ALLOCATABLE :: tdens(:,:,:,:,:)
...
!--------------------------------------------------------------------------- 
  SUBROUTINE alloc_densities
    ALLOCATE(rho(nx,ny,nz,3),tau(nx,ny,nz,3),current(nx,ny,nz,3,2), &
         sdens(nx,ny,nz,3,2),sodens(nx,ny,nz,3,3),tdens(nx,ny,nz,3,2))
...
!> @param[in, out] ltdens
!> REAL(db), array, takes and adds the spin-kinetic density.
!--------------------------------------------------------------------------- 
  SUBROUTINE add_density(iq,weight,psin,lrho,ltau,lcurrent,lsdens,lsodens,ltdens)  
    COMPLEX(db),INTENT(INOUT) :: psin(nx,ny,nz,2)
    REAL(db),DIMENSION(:,:,:,:),INTENT(INOUT) :: lrho,ltau
    REAL(db),DIMENSION(:,:,:,:,:),INTENT(INOUT) :: lcurrent,lsdens,lsodens,ltdens
...
if(iq<3) THEN
    ...
    FORALL(iz=1:nz,iy=1:ny,ix=1:nx)
     ltdens(ix,iy,iz,1,iq)=ltdens(ix,iy,iz,1,iq)+2.D0*weight &
       *REAL(CONJG(ps1(ix,iy,iz,1))*ps1(ix,iy,iz,2))
     ltdens(ix,iy,iz,2,iq)=ltdens(ix,iy,iz,2,iq)+2.D0*weight &
       *AIMAG(CONJG(ps1(ix,iy,iz,1))*ps1(ix,iy,iz,2))
     ltdens(ix,iy,iz,3,iq)=ltdens(ix,iy,iz,3,iq)+weight &
       *(REAL(CONJG(ps1(ix,iy,iz,1))*ps1(ix,iy,iz,1)) &
       -REAL(CONJG(ps1(ix,iy,iz,2))*ps1(ix,iy,iz,2)))
     END FORALL
endif
    !***********************************************************************
    ! y-derivatives
